...
(async () => {
  ...
  const htmlPath = path.resolve(__dirname, 'escopo-agroterra-apside.html');
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    const filename = `escopo-apside-pagina-${String(i + 1).padStart(2, '0')}.png`;
    fs.writeFileSync(path.resolve(__dirname, filename), buf);
    console.log(`Gerado ${filename} (${buf.length} bytes)`);
  }

  await page.pdf({
    path: path.resolve(__dirname, 'escopo-agroterra-apside.pdf'),
    landscape: true,
    format: 'A4',
    printBackground: true,
    margin: { top: '0', right: '0', bottom: '0', left: '0' }
  });
  console.log('Gerado escopo-agroterra-apside.pdf');
