...
(async () => {
  ...
  fs.writeFileSync(path.resolve(outDir, 'prospeccao-completa.png'), fullBuf);
  console.log(`Gerado prospeccao-completa.png (${fullBuf.length} bytes)`);

  await page.pdf({
    path: path.resolve(outDir, 'prospeccao-clinicas.pdf'),
    landscape: true,
    format: 'A4',
    printBackground: true,
    margin: { top: '0', right: '0', bottom: '0', left: '0' }
  });
  console.log('Gerado prospeccao-clinicas.pdf');
